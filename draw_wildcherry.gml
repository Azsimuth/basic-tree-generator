///draw_wildcherry(int tree_height)
// input
...
var folliage_passes, folliage_count;
    ...
    for (m=0;m<ds_list_size(major_branches)/2;m+=2) {
        ...
        for (n=0;n<folliage_passes;n+=1) {
                used_spr = choose(spr_folliage_main, spr_folliage_greener, spr_folliage_darker, spr_folliage_yet_darker, spr_folliage_greens);
                var k;
                for (k=0;k<folliage_count;k+=1) {
                    var subimg; subimg = irandom(sprite_get_number(used_spr));
                    if (k==folliage_count-1) {used_spr = spr_folliage_main}
                    var dx, dy; 
